...
run_docker_mode() {
  ...
  step "Generating .env and starting the Docker stack"

  # Auto-generate .env from user choices so beginners only need to edit one file
  if [ ! -f .env ] || prompt_yes_no "Overwrite existing .env with current settings?" "y"; then
    cat > .env <<ENVEOF
# MarkOS UI — generated by install.sh ($(date +%Y-%m-%d))
# Edit this file, then run: docker compose down && docker compose up -d

MARKOS_UI_PORT=${UI_PORT}
OPENCLAW_UPSTREAM_HOST=${DOCKER_UPSTREAM_HOST}
OPENCLAW_UPSTREAM_PORT=${GATEWAY_PORT}
ENVEOF
    success "Generated .env (UI port=${UI_PORT}, gateway=${DOCKER_UPSTREAM_HOST}:${GATEWAY_PORT})"
  fi
  ...
  echo -e "  ${GREEN}Config file:${NC} $(pwd)/.env"
  echo ""

...
function isCronJob(value: unknown): value is CronJob {
  return isRecord(value) &&
    ['cron', 'every', 'at'].includes(value.scheduleType as string) &&
    ['ok', 'error', 'pending'].includes(value.status as string) &&
    typeof value.enabled === 'boolean' &&
    ['id', 'name', 'schedule', 'agentId', 'sessionTarget'].every((field) => typeof value[field] === 'string') &&
    (value.lastRun === undefined || typeof value.lastRun === 'string') &&
    (value.nextRun === undefined || typeof value.nextRun === 'string') &&
...
export default function Cron() {
  ...
  const { data: cronJobs, isLive, refetch } = useGatewayData<CronJob[]>('cron.list', {}, mockCronJobs, 15000)
  const { data: sessionsRaw } = useGatewayData<unknown>('sessions.list', {}, mockSessions, 10000)
  const { data: presenceRaw } = useGatewayData<unknown>('system-presence', {}, { agents: mockAgents }, 10000)
  const sessions = normalizeSessions(sessionsRaw, mockSessions)
  const agents = normalizeAgents(presenceRaw, sessions, mockAgents)

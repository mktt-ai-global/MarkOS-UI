...
class OpenClawClient {
  ...
  private async buildSignedDevice(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const deviceFamily = this.getDeviceFamily()
    const platform = this.getClientPlatform()
  ...
  // Convenience methods
  // TODO: verify RPC method names against a live gateway.
  // 'node.list' uses singular while other list methods use plural (sessions.list, models.list) --
  // confirm whether the gateway expects 'node.list' or 'nodes.list'.
  async getGatewayStatus() { return this.rpc('status') }
  async getHealth() { return this.rpc('health') }
  async getChatHistory(sessionKey?: string) { return this.rpc('chat.history', sessionKey ? { sessionKey } : {}) }
  async sendChat(text: string, sessionKey?: string) { return this.rpc('chat.send', { text, sessionKey }) }
  async abortChat(sessionKey?: string) { return this.rpc('chat.abort', sessionKey ? { sessionKey } : {}) }
  async getConfig(key?: string) { return this.rpc('config.get', key ? { key } : {}) }
  async setConfig(key: string, value: unknown) { return this.rpc('config.set', { key, value }) }
  async patchConfig(patch: Record<string, unknown>) { return this.rpc('config.patch', { patch }) }
  ...
  async getSkillsBins() { return this.rpc('skills.bins') }
  async getPresence() { return this.rpc('system-presence') }
  async tailLogs() { return this.rpc('logs.tail') }

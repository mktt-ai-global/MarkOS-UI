...
function normalizeSkillCategory(record: UnknownRecord): SkillInfo['category'] {
  ...
  if (name.includes('api') || name.includes('search')) return 'api'
  if (name.includes('custom')) return 'custom'
  if (name.includes('shell') || name.includes('runtime') || name.includes('sandbox')) return 'system'
  return 'tool'
}
...
export function normalizeSessions(raw: unknown, fallback: ChatSession[] = mockSessions): ChatSession[] {
  ...
  const normalized = items
    .map((item, index) => {
      ...
      const agentId = asString(getValue(item, ['agentId', 'agent_id', 'agent']), `agent-${index + 1}`)
      const agent = asString(getValue(item, ['agent', 'agentName', 'name']), agentId)
      const title = asString(getValue(item, ['title', 'name', 'label', 'subject']), agent)
      const lastMessage = asString(getValue(item, ['lastMessage', 'preview', 'snippet']), 'No messages yet')
      const timestamp = asString(getValue(item, ['timestamp', 'updatedAt', 'lastMessageAt', 'createdAt']), 'Recently')
      ...
      return {
        ...
        kind,
        model: asString(getValue(item, ['model', 'modelId', 'providerModel']), 'unknown'),
...
export function normalizePresence(
  ...
  const agentItems = findArray(record, ['agents', 'workers', 'operators'])
  const totalAgents = asNumber(getValue(record, ['totalAgents', 'agentCount']), agentItems.length || fallback.totalAgents)
  const activeAgents = asNumber(
    getValue(record, ['activeAgents', 'runningAgents']),
    agentItems.filter((item) => isRecord(item) && normalizeAgentStatus(getValue(item, ['status', 'state'])) === 'active').length || fallback.activeAgents,
  )
  ...
  return {
    ...
    totalAgents,
    activeAgents,
    activeSessions: asNumber(getValue(record, ['activeSessions', 'sessionCount']), sessions.length || fallback.activeSessions),
    skillsDeployed: asNumber(getValue(record, ['skillsDeployed', 'skillCount']), skills.length || fallback.skillsDeployed),
...
export function normalizeAgents(
  ...
  const presenceItems = findArray(presenceRaw, ['agents', 'workers', 'operators', 'items'])
  const fallbackById = new Map(fallback.map((agent) => [agent.id, agent]))

  const fromPresence: AgentInfo[] = []
  for (const [index, item] of presenceItems.entries()) {
    if (!isRecord(item)) continue

    const id = asString(getValue(item, ['id', 'agentId', 'name']), `agent-${index + 1}`)
    const fallbackAgent = fallbackById.get(id)
    const sessionsCount = asNumber(getValue(item, ['sessions', 'sessionCount']), fallbackAgent?.sessions ?? 0)
    const tokenCount = asNumber(getValue(item, ['tokensUsed', 'tokenCount', 'tokens']), 0)

    fromPresence.push({
      id,
      name: asString(getValue(item, ['name', 'label', 'agentName']), fallbackAgent?.name ?? id),
      model: asString(getValue(item, ['model', 'modelId']), fallbackAgent?.model ?? 'unknown'),
      status: normalizeAgentStatus(getValue(item, ['status', 'state'])),
      sessions: sessionsCount,
      tokensUsed: tokenCount > 0 ? compactNumber(tokenCount) : (fallbackAgent?.tokensUsed ?? '0'),
      successRate: asNumber(getValue(item, ['successRate', 'healthScore']), fallbackAgent?.successRate ?? 92),
      uptime: asString(getValue(item, ['uptime']), fallbackAgent?.uptime ?? 'Unknown'),
      lastActive: asString(getValue(item, ['lastActive', 'updatedAt']), fallbackAgent?.lastActive ?? 'Recently'),
      workspace: asString(getValue(item, ['workspace', 'path', 'cwd']), fallbackAgent?.workspace ?? '~/.openclaw/workspace'),
...
export function normalizeSkills(
  ...
  const mergeSkill = (record: UnknownRecord, installedHint: boolean) => {
    ...
    byId.set(id, {
      id,
      name: asString(getValue(record, ['name', 'title']), previous?.name ?? fallbackSkill?.name ?? id),

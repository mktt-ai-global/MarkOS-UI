...
export default function Agents() {
  ...
  const { data: presenceRaw, isLive: presenceLive } = useGatewayData<unknown>('system-presence', {}, { agents: mockAgents }, 10000)
  const sessions = normalizeSessions(sessionsRaw, mockSessions)
  const liveAgents = normalizeAgents(presenceRaw, sessions, mockAgents)
  ...
  const isLive = sessionsLive || presenceLive
  ...
  const pendingGatewayTodos = [
    'Create agents in the live OpenClaw runtime from the web UI',
    'Start and pause agents through gateway RPC',
    'Open editable agent settings and model config',
    'Replace preview performance history with real telemetry',
  ]

  const handlePendingAction = (label: string) => {
    setActionMessage(`${label} is still on the TODO list for real gateway control. It needs a live OpenClaw install and verified RPC wiring.`)
  }
  ...
  const handleCreateLocalDraft = (draft: AgentInfo, form: AgentTemplateForm) => {
    ...
    setShowCreator(true)
    setActionMessage(`Saved local template "${draft.name}". Installing this agent into a live OpenClaw runtime remains a TODO.`)
  }
  ...
  return (
    <div className="space-y-3 md:space-y-4 animate-fade-in">
      ...
      <GlassCard title="Pending Gateway Control" subtitle="Safe to defer until OpenClaw is installed locally">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {pendingGatewayTodos.map((item) => (
            ...
            <GlassCard variant="strong">
              ...
              <div className="space-y-3 mb-4">
                {selectedAgent.model.endsWith('/draft') && (
                  <div className="rounded-xl px-3 py-2 text-xs bg-info/10 text-info">
                    This is a local template created from Template Studio. Runtime install and activation stay TODO until a live OpenClaw gateway is available.
                  </div>
                )}
                {selectedDraft && (

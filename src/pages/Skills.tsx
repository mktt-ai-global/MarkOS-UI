...
export default function Skills() {
  ...
  const liveSkills = normalizeSkills(skillsBinsRaw, toolsCatalogRaw, mockSkills)
  ...
  const isLive = skillsBinsLive || toolsCatalogLive
  const editingDraft = localDraftSkills.find((draft) => draft.preview.id === editingDraftId) || null
  const pendingGatewayTodos = [
    'Install and remove skills in the live OpenClaw runtime from the web UI',
    'Persist custom skills into the runtime from this UI',
    'Validate runtime dependencies against a local OpenClaw install',
    'Replace mock dependency health with real package/runtime checks',
  ]
  ...
  const openStudioForEdit = (draftId: string) => {
    ...
    setTab('custom')
    setShowCreator(true)
    setActionMessage(`Editing local template "${draftId}". Runtime install/save still stays on the TODO list.`)
  }
  ...
  const handleCreateLocalDraft = (draft: SkillInfo, form: SkillTemplateForm) => {
    ...
    setShowCreator(true)
    setActionMessage(`Saved local template "${draft.name}". Runtime install/save still stays on the TODO list.`)
  }
  ...
  return (
    <div className="space-y-3 md:space-y-4 animate-fade-in">
      ...
      <GlassCard title="Pending Skill Management" subtitle="These items stay as TODO until a real OpenClaw runtime is available">

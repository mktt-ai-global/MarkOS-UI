...
export default function Chat() {
  ...
  const simulateResponse = (userText: string, sessionKey: string, agentName: string) => {
    pendingResponseTimerRef.current = setTimeout(() => {
      const assistantMsg: ChatMessage = {
        id: nextLocalMessageId(),
        role: 'assistant',
        content: `I received your message: "${userText}"\n\n*This is an offline simulated response. The gateway is not connected.*`,
        timestamp: createTimestamp(),
        agent: agentName,
      }
  ...
  const handleStartFromLocalTemplate = (template: LocalAgentDraft) => {
    if (sessionsLive || connectionStatus === 'connected') {
      setUiMessage('Starting a new session from a local template while connected to the gateway is not yet supported. Disconnect to use offline templates.')
      return
    }
  ...
  const handleInjectMessage = () => {
    ...
    if (messagesLive || connectionStatus === 'connected') {
      setUiMessage('Injecting system messages into a live session is not yet supported.')
      return
    }
    ...
    <div className="flex gap-3 md:gap-4 h-[calc(100vh-8rem)] md:h-[calc(100vh-8rem)] animate-fade-in">
      ...
      <div className="hidden md:flex w-64 lg:w-72 flex-shrink-0 flex-col gap-3">
        <div className="flex items-center gap-2">
          <div className="flex-1 flex items-center gap-2 px-3 py-1.5 rounded-xl glass-input text-xs">
      ...
      <div className="flex-1 flex flex-col glass rounded-2xl overflow-hidden">
        ...
        <div className="mx-4 md:mx-5 mt-2 space-y-2">
          {!sessionsLive && localAgentTemplates.length > 0 && (
            <div className="md:hidden glass-subtle rounded-xl p-2 space-y-1.5">
              <div className="text-[10px] font-medium text-text-tertiary">Quick Launch</div>
              <div className="flex gap-1.5 overflow-x-auto">
                {filteredLocalTemplates.slice(0, 3).map((template) => (
                  <button
                    key={`mobile-${template.preview.id}`}
                    onClick={() => handleStartFromLocalTemplate(template)}
                    className="flex-shrink-0 px-2.5 py-1.5 rounded-lg bg-[var(--color-glass-subtle)] hover:bg-[var(--color-glass)] transition-colors text-[10px] text-text-primary"
                  >
          ...
          {/* Collapsible session panel toggle */}
          <button
            onClick={() => setShowSessionPanel((prev) => !prev)}
            className="w-full flex items-center justify-between px-3 py-1.5 rounded-xl glass-subtle text-[10px] text-text-secondary hover:text-text-primary transition-colors"
          >
            <div className="flex items-center gap-2">
              <span>Session: {activeSession?.agentId || 'unknown'}</span>
              <span className="text-text-tertiary">{activeSession?.model}</span>
              <span className="text-text-tertiary">{sessionUsagePercent}% used</span>
              {overrideChangeCount > 0 && (
                <span className="px-1.5 py-0.5 rounded-full bg-info/10 text-info">{overrideChangeCount} override{overrideChangeCount === 1 ? '' : 's'}</span>
              )}
              {isSending && <span className="px-1.5 py-0.5 rounded-full bg-warning/10 text-warning">running</span>}
            </div>
            {showSessionPanel ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
          </button>
            ...
            <div className="glass-subtle rounded-2xl p-3">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  >
                    {showInjectComposer ? 'Hide Inject' : 'Inject Note'}
                  </button>
                </div>
              </div>
                ...
                <div className="mt-3 rounded-xl bg-[var(--color-glass-subtle)] p-3 space-y-2 animate-fade-in">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div className="text-[10px] text-text-tertiary">
                      {connectionStatus === 'connected'
                        ? 'Inject a system message into the live session (not yet supported for live sessions).'
        ...
        <div className="px-4 md:px-5 py-3 border-t border-[var(--color-glass-border)]">
          <div className="flex items-end gap-2">
            ...
            <div className="flex-1 glass-input rounded-2xl px-4 py-2.5 flex items-end gap-2">
              <textarea
                ref={textareaRef}
                value={inputValue}

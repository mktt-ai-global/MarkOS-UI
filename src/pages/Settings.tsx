...
export default function Settings() {
  ...
  const installBlockedTodos = [
    'Validate browser device signing and pairing against a real OpenClaw gateway',
    'Verify config.patch / config.apply writes against a real OpenClaw gateway',
    'Wire live Devices and Approvals actions for pairing and exec approval flows',
    'Wire real agent lifecycle controls and skill install/remove actions',
  ]
        ...
        <div className="flex-1 space-y-4">
          ...
          {activeTab === 'general' && (
            <>
                                ...
                                <label key={field.path} className="block">
                                  ...
                                  {field.kind === 'enum' ? (
                                    ...
                                    <input
                                      ...
                                      value={typeof fieldValue === 'string' ? fieldValue : ''}
                                      onChange={(event) => handleDraftFieldChange(field.path, event.target.value)}
                                      className={`w-full glass rounded-xl px-3 py-2 text-sm text-text-primary outline-none ${isInvalid ? 'ring-2 ring-danger/30' : ''}`}
                                    />
                                  )}
              ...
              <GlassCard
                ...
                subtitle="Read-only for now; write/apply stays on the TODO list until a real gateway is available"
                action={(
          ...
          {activeTab === 'about' && (
            <>
              ...
              <GlassCard title="Pending Real Gateway Validation" subtitle="Keep these as TODO until OpenClaw is installed on this machine">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {installBlockedTodos.map((item) => (

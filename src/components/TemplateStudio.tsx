  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className={`w-full glass-subtle rounded-xl px-3 py-2 text-sm text-text-primary outline-none focus:ring-2 focus:ring-accent/20 ${error ? 'ring-2 ring-red-400' : ''}`}
      />
  ...
  return (
    <label className="block">
      ...
      <textarea
        ...
        placeholder={placeholder}
        rows={rows}
        className="w-full glass-subtle rounded-xl px-3 py-2 text-sm text-text-primary outline-none focus:ring-2 focus:ring-accent/20 resize-y"
      />
...
export default function TemplateStudio({
  ...
  const handleCreateLocalDraft = () => {
    ...
    if (mode === 'agent') {
      ...
      setStudioMessage(`Saved local agent template "${draft.name}". Runtime install / activation remains a TODO.`)
      return
    }

    const draft = buildSkillDraft(skillForm)
    onCreateSkillDraft?.(draft, skillForm)
    setStudioMessage(`Saved local skill template "${draft.name}". Runtime install remains a TODO.`)
  }
        ...
        <div className="space-y-4">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
            ...
            <div className="rounded-xl px-3 py-2 text-xs bg-info/10 text-info">
              Runtime save / install stays TODO. This studio focuses on scaffold generation, import, preview, and local templates.
            </div>
          </div>
